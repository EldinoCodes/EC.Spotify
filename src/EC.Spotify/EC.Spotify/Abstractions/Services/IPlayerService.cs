...
public interface IPlayerService
{
    ...
    /// </summary>
    /// <remarks>If the specified device is not active or available, the operation may fail. The method does
    ...
    /// </summary>
    /// <remarks>If the specified device is not available or cannot be transferred to, the operation may fail.
    ...
    Task<SpotifyResult<bool>> PlayerPlayAsync(string? deviceId, List<string>? trackUris, CancellationToken cancellationToken = default);
    /// <summary>
    ...
    Task<SpotifyResult<bool>> PlayerPauseAsync(string? deviceId = null, CancellationToken cancellationToken = default);
    /// <summary>
    /// Skips to the next track in the user's currently active Spotify player.
    /// </summary>
    /// <remarks>This method requires the user to have an active Spotify playback session. If no device is
    ...
    Task<SpotifyResult<bool>> PlayerNextAsync(string? deviceId = null, CancellationToken cancellationToken = default);    
    /// <summary>
    /// Skips to the previous track in the user's currently active Spotify player.
    /// </summary>
    /// <remarks>This method requires the user to be authenticated and have an active Spotify playback
    ...
    Task<SpotifyResult<bool>> PlayerPreviousAsync(string? deviceId = null, CancellationToken cancellationToken = default);

    /// <summary>
    /// Seeks to the specified position in the currently playing track on the user's Spotify player.
    /// </summary>
    /// <remarks>If playback is not active or the specified position exceeds the track's duration, the
    ...
    Task<SpotifyResult<bool>> PlayerSeekAsync(int positionMs, string? deviceId = null, CancellationToken cancellationToken = default);
    /// <summary>
    /// Sets the repeat mode for the current Spotify playback session asynchronously.
    /// </summary>
    /// <remarks>If the specified device is not active or available, the operation may fail. This method does
    ...
    Task<SpotifyResult<bool>> PlayerRepeatAsync(PlayerRepeatMode playerRepeatMode = PlayerRepeatMode.Off, string? deviceId = null, CancellationToken cancellationToken = default);
    /// <summary>
    /// Enables or disables shuffle mode for the user's playback on Spotify.
    /// </summary>
    /// <remarks>If no device is specified, the shuffle setting is applied to the user's currently active
    /// device. The operation requires the user to be authenticated and may fail if playback is not active.</remarks>
    ...
    Task<SpotifyResult<bool>> PlayerShuffleAsync(PlayerShuffleMode playerShuffleMode = PlayerShuffleMode.Off, string? deviceId = null, CancellationToken cancellationToken = default);
    /// <summary>
    /// Sets the playback volume for the specified Spotify device asynchronously.
    /// </summary>
    /// <remarks>If the device is not active or does not support volume control, the operation may fail. This
    ...
    Task<SpotifyResult<bool>> PlayerVolumeAsync(int volumePercent, string? deviceId = null, CancellationToken cancellationToken = default);
}